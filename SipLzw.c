...
void ETSipEncode(char* o_src, int len, char*dst, int *outlen)
{
          char *temp_1 = (char*)malloc(sizeof(char)*(*outlen));
          char *temp_2 = (char*)malloc(sizeof(char)*(*outlen));
          memset(temp_1, 0, *outlen);
          memset(temp_2, 0, *outlen);
          memcpy(temp_1, o_src, strlen(o_src));

          char* src = o_src;
          int outlength = *outlen;
          //int n = length(DirectorySrc);
          int n = sizeof(DirectorySrc)/sizeof(DirectorySrc[0]);
          int i=0;
          for(; i<n; i++)
          {
              strrpl( temp_1, temp_2, *outlen, DirectorySrc[i].key, DirectorySrc[i].value) ;
              char *now = temp_2;
              temp_2 = temp_1;
              temp_1 = now;
              memset(temp_2, 0, *outlen);
         ...
         byte *in = _new(char, strlen(temp_1));
         memcpy( in, temp_1, strlen(temp_1));
         _setsize(in, strlen(temp_1));
 
         byte *enc = lzw_encode(in, 9);
        
         memset(dst, 0, outlength);
         memcpy(dst, enc-(sizeof(size_t)*2), _len(enc)+(sizeof(size_t)*2));
         *outlen = _len(enc) + (sizeof(size_t)*2);
...
void ETSipDecode(char* src, int len, char*dst, int* outlen)
{
          char *temp_1 = (char*)malloc(sizeof(char)*(*outlen));
          char *temp_2 = (char*)malloc(sizeof(char)*(*outlen));
          memset(temp_1, 0, *outlen);
          ...
          int lengthTemp = *outlen;

         byte *dec = lzw_decode( (byte*)(src + sizeof(size_t)*2) );

         memcpy(temp_1, dec, _len(dec) );
          ...
          for(; i<n; i++)
          {
              strrpl( temp_1,  temp_2, *outlen, DirectorySrc[i].value, DirectorySrc[i].key) ;
              char *now = temp_1;
              temp_1 = temp_2;
              temp_2 = now;
              memset(temp_2, 0, *outlen);
          ...
          memcpy(dst, temp_1, strlen(temp_1));
          *outlen = strlen(temp_1);
...
int main()
{
    ...
    char *in = malloc(sizeof(char) *(st.st_size + 1));
    ...
    char buf[4096]={'\0'};
    int len = 4096;
    ETSipEncode( in, st.st_size, buf, &len);
    
    printf("encode  is [%d][%s]\n", len, buf);

    char outbuf[4096]={'\0'};
    int outlen = 4096;
    ETSipDecode(buf, len, outbuf, &outlen);

    printf("decode is  [%d][%s] \n", outlen, outbuf);
